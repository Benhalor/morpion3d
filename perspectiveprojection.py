...
class Space:
    ...
    def __init__(self, lightvector = (2,3,1)):
        self.__xAxis = (20, 0, 0)
        self.__yAxis = (0, 20, 0)
        self.__zAxis = (0, 0, 20)
        self.__originx = 320
        ...
        self.__lightVector = Vector(self, lx, ly, lz)
        Space.index += 1
...
class Point:
    ...
    def update(self):
        ...
        self.__virtualx = self._space.cy * (
                self._space.sz * self.__y + self._space.cz * self.__x) - self._space.sy * self.__z
        self.__virtualy = self._space.sx * (self._space.cy * self.__z + self._space.sy * (
                self._space.sz * self.__y + self._space.cz * self.__x)) + self._space.cx * (
                                  self._space.cz * self.__y - self._space.sz * self.__x)
        self.__virtualz = self._space.cx * (self._space.cy * self.__z + self._space.sy * (
                self._space.sz * self.__y + self._space.cz * self.__x)) - self._space.sx * (
                                  self._space.cz * self.__y - self._space.sz * self.__x)

        # projection in the 2D plane
        axes = self._space.axes
        origin = self._space.origin
        self.__projectedx = self.__virtualx * axes[0][0] + self.__virtualy * axes[1][0] + self.__virtualz * axes[2][0] + origin[0]
        self.__projectedy = self.__virtualx * axes[0][1] + self.__virtualy * axes[1][1] + self.__virtualz * axes[2][1] + origin[1]
...
class Vector(Point) :
    ...
    def __scalar_product_with_light(self):
        """Scalar product divided by the norms of the vectors of this vector with the light vector form the space instance"""
        norms = self.norm*self._space.lightVector.norm
        if norms != 0 :
            (x1,y1,z1) = self.xyzVirtual
            (x2,y2,z2) = self._space.lightVector.xyzTrue
            return (x1*x2+y1*y2+z1*z2)/norms
        else :
            return 0

    def __color_coeff(self):
        """Computes a color coefficient corresponding to the scalar product of the vector with a light vector"""
        return 0.5*(1-self.__scalar_product_with_light())
    color_coeff = property(__color_coeff)
