...
class Server(Thread):
    def __init__(self, port):
        ...
        self.matrixSize = 3 #assume it is a square matrix
        self.dimension = 2
        self.matrix = np.zeros([self.matrixSize for i in range(self.dimension)])
    ...
    def connect_clients (self):
        ...
        for i in range(self.numberOfPlayers):
            #id/dimension/size #assume it is a square matrix
            tempConnection , tempsInfoConnection = self.main_connection.accept()
            command = str(self.idCounter)+"/"
            command += str(self.dimension)+ "/"
            command += str(self.matrixSize)
